...
int sysmon_uid_read(char *buffer, char **buffer_location, off_t offset, int buffer_lenght, int *eof, void *data){
	...
	memcpy(buffer, (char *)data, SYSMON_UID_MAXSIZE);

        return SYSMON_UID_MAXSIZE;
}
...
int sysmon_toggle_read(char *buffer, char **buffer_location, off_t offset, int buffer_lenght, int *eof, void *data){
	...
	memcpy(buffer, (char *)data, SYSMON_TOGGLE_MAXSIZE);

        return SYSMON_TOGGLE_MAXSIZE;
}


int sysmon_uid_wite(struct file *file, const char *buffer, unsigned long count, void *data){


	char *tempBuffer;

	count = SYSMON_UID_MAXSIZE;						//done to avoid problem in reading by the size of the read buffer
